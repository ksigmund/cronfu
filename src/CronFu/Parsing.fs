...
let pCronExpression =
    parse {
        let! (minutes : CronMinute list) = (pCronInt .>> spaces1)
        let! (hours: CronHour list) = (pCronInt .>> spaces1)
        let! (days: CronDayOfMonth list) = (pCronInt .>> spaces1)
        let! (months: CronMonth list) = (pCronIntStr .>> spaces1)
        let! (dow: CronDayOfWeek list) = (pCronIntStr .>> eof)
